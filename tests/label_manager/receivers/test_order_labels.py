...
"""
from disseminate.label_manager.receivers import order_labels
...
def test_order_labels_processor(doctree):
    ...
    label3 = Label(doc_id=doc2.doc_id, id='label4', kind=('content', 'figure'),
                   mtime=None)
    label4 = Label(doc_id=doc2.doc_id, id='label5', kind=('content', 'figure'),
                   mtime=None)

    # 1. Try setting the order of the labels. First, try it by excluding table
    #    labels
    registered_labels = [label1, label2, label3, label4]
    order_labels(registered_labels=registered_labels,
                 root_context=doc1.context)
